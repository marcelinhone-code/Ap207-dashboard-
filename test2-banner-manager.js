...
let current=0,timer=null,spriteReady=false,cozeeReady=false;
...
#t2New27Overlay{position:absolute!important;inset:0!important;z-index:999!important;border-radius:inherit!important;overflow:hidden!important;background-color:#dfe7ef!important;background-repeat:no-repeat!important;background-position:center center!important}
...
function ensureOverlay(){const h=document.querySelector('.scenic-banner');if(!h)return null;h.style.position='relative';h.style.overflow='hidden';let o=document.getElementById('t2New27Overlay');if(!o){o=document.createElement('div');o.id='t2New27Overlay';h.append(o);applyCozee(o)}return o}
function showSlide(){const o=ensureOverlay();if(!o)return;if(!spriteReady){applyCozee(o);return}const slides=[...Array.from({length:27},(_,i)=>({type:'sprite',i})),{type:'cozee'}];const s=slides[current%slides.length];current=(current+1)%slides.length;if(s.type==='sprite'){applySlice(o,s.i);o.setAttribute('aria-label',`Imagem nova ${s.i+1} de 27`)}else applyCozee(o)}
function start(){if(timer)clearInterval(timer);const o=ensureOverlay();if(o)applyCozee(o);current=0;timer=setInterval(showSlide,ROTATE_MS)}
function preload(){const o=ensureOverlay();if(o)applyCozee(o);const c=new Image();c.onload=()=>{cozeeReady=true};c.onerror=()=>{cozeeReady=false};c.src=COZEE;const p=new Image();p.onload=()=>{spriteReady=true;showSlide()};p.onerror=()=>{spriteReady=false;if(o)applyCozee(o)};p.src=SPRITE}
function refresh(){css();renderManager();ensureOverlay();start();preload()}
function boot(){refresh();window.addEventListener('stay:unified-navigation',e=>{if(e.detail?.route==='publicity')setTimeout(renderManager,20)});window.addEventListener('t2-banners-updated',()=>{setTimeout(()=>{renderManager();const o=ensureOverlay();if(o&&!spriteReady)applyCozee(o)},20)});const mo=new MutationObserver(()=>{if(document.querySelector('.scenic-banner')&&!document.getElementById('t2New27Overlay')){const o=ensureOverlay();if(o)applyCozee(o)}});mo.observe(document.body,{childList:true,subtree:true})}
