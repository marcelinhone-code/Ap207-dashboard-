...
body[data-t2-route="calendar"] .calendar-legend .free{background:#fff4b8!important;border:2px solid #c58a00!important}
body[data-t2-route="calendar"] .calendar-legend .reserved{background:#ddd6fe!important;border:2px solid #6d28d9!important}
body[data-t2-route="calendar"] .calendar-legend .checkin{background:#bbf7d0!important;border:2px solid #15803d!important}
body[data-t2-route="calendar"] .calendar-legend .checkout{background:#fed7aa!important;border:2px solid #c2410c!important}
body[data-t2-route="calendar"] .calendar-day{background:#fffdf5!important;border-color:#d1d5db!important;color:#374151!important}
body[data-t2-route="calendar"] .calendar-day.is-reserved{background:#ddd6fe!important;border-color:#7c3aed!important;color:#4c1d95!important}
body[data-t2-route="calendar"] .calendar-day.is-checkin{box-shadow:inset 6px 0 0 #15803d!important}
body[data-t2-route="calendar"] .calendar-day.is-checkout{box-shadow:inset -6px 0 0 #c2410c!important}
...
function moveNode(n,g){if(n&&g&&n!==g&&n.parentNode!==g)g.append(n)}
function move(id,g){moveNode($(id),g)}
function movePrimary(name,g){moveNode(q(`.app-screen[data-screen-panel="${name}"]`),g)}
...
function organize(){classifyLegacy();if(role()==='owner')return;const gp=group('t2UnifiedProperties'),gf=group('t2UnifiedFinancial'),gc=group('t2UnifiedContracts'),gs=group('t2UnifiedSettings');
...
move('t2ProfessionalSettings',gs);support(gs)}
function managedRoots(){const out=[];qa('.app-screen').forEach(x=>out.push(x));qa('#t2Suite > section').forEach(x=>out.push(x));qa('main.container > section.panel').forEach(x=>out.push(x));['t2UnifiedProperties','t2UnifiedFinancial','t2UnifiedContracts','t2UnifiedSettings'].forEach(id=>{const x=$(id);if(x)out.push(x)});return[...new Set(out)]}
function hideEverything(){managedRoots().forEach(x=>{x.hidden=true;x.style.display='none'})}
function showNode(n){if(!n)return;n.hidden=false;n.style.display=''}
function showKnownChildren(k,t){showNode(t);if(k==='properties'){['stayUserSection','propertySettings','t2ProfessionalProperties','t2PropertyAssignments','t2PropertyLinking','t2ProfessionalOwners'].forEach(id=>showNode($(id)))}
else if(k==='expenses'){showNode(q('.app-screen[data-screen-panel="expenses"]'));['t2Operations','t2FeatureExtras'].forEach(id=>showNode($(id)))}
...
else if(k==='calendar'){qa(':scope > section.panel',t).forEach(showNode)}
else if(k==='reports'){qa(':scope > section.panel',t).forEach(showNode)}}
...
function boot(){css();paintBanner();labels();organize();initializePeriods();document.addEventListener('click',click,true);let queued=false;const obs=new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;const changed=!overviewInitialized;organize();labels();paintBanner();initializePeriods();if(changed&&overviewInitialized&&validRoute((location.hash||'').slice(1))==='home')show('home',false)})});obs.observe(document.body,{childList:true,subtree:true});show((location.hash||'').slice(1)||'home',false);window.addEventListener('pageshow',()=>{overviewInitialized=false;initializePeriods();show((location.hash||'').slice(1)||'home',false)});window.Test2Unified={show,organize,initializePeriods,refreshCalendar}}
